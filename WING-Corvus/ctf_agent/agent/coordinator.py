"""/29: 巡查指导器 (Coordinator LLM) — 智能旁观者.
...
5. **无充分证据禁止否定方向 (强化, CTF 漏洞寻找阶段)**: 在寻找漏洞
...
class Coordinator:
    ...
    def __init__(
        ...
        execution_starvation_min_steps: int = 20,  # 分析瘫痪检测启用步数 (前期信息收集合理)
    ...
    def report_recon_done(self, summary: str = "") -> bool:
        """汇报该路 P1 侦查完成 (三路全部完成后总指挥整合全局情报 → P2).
    ...
    def check_commander_directives(self) -> CoordinatorGuidance | None:
        ...
        WING-Corvus 升级 (2026-08-05, 历史复盘): MUST 本地冲突校验 —
        ...
        指令附带 phase (P1/P2/P3/P4) — 更新战略层的阶段感知,
    ...
    def analyze(
        ...
        # (历史复盘: 协调器 step10 下达 MUST 但 agent 忽略, 继续 MD5 爆破 20 步)
        must_ignored = self._check_must_noncompliance(recent)
        if must_ignored:
            hard_issues.append(must_ignored)

        # 复盘: 分析瘫痪 — 长时间无执行类工具 (只读源码/解析数据/静态分析)
    ...
    def _has_progress_after_guidance(self, recent: list[dict]) -> bool:
        ...
        修复 (某解题器 innovative 死循环): 旧逻辑只检查 observation 非空,
    ...
    def _query_knowledge(self, task_desc: str, challenge_type: str,
        ...
        (题目混淆修复): 匹配文本从 task_desc 改为**轨迹实际观测**
