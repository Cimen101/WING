...
- ✅ **信息收集第 1-3 步必须执行**: `ls -la /challenge/workspace/` 或 `ls /challenge/` +
  `find /challenge -maxdepth 2 -type f` — 确认是否存在附件 (README.md / vm_spec.txt /
  加密脚本 / 样本文件等)
- ✅ **附件存在则必须读取**: `cat /challenge/workspace/<file>` 逐文件阅读,
  附件 (如 README.md / vm_spec.txt / 加密脚本) 往往直接给出协议说明、指令集参考、
  算法细节 — 不读附件等于自断关键线索
- ✅ 附件是**解题辅助文档**, 不是"外部题解" — 允许读取 (与本轮 FlagVerifier 拦截的
  外部 GitHub writeup 不同)
- ⛔ **不要跳过附件直接盲连靶机**: ouroboros 复盘 — 三路 agent 均未读已挂载的
  README.md/vm_spec.txt, 在 /api/files 枚举上耗尽时间; cornflake 复盘 — 未读
...
def build_system_prompt(
    ...
    *,
    ...
    if inject_skills and task:
        try:
            skill_text = _try_inject_skills(task, challenge_type, difficulty, skill_max_chars)
