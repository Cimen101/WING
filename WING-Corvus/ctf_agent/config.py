...
class Settings(BaseSettings):
    ...
    # false → 回退早期结论 (仅 medium/hard 并行, easy 单路).
    swarm_enabled: bool = Field(default=True, alias="SWARM_ENABLED")
    # 总指挥 (Commander) 开关 — 三层协作小队 (总指挥+战略层+战术层).
    # 默认 false = 纯雁阵行为不变 (升级验证期间), 开启后 swarm 启动总指挥实例:
    # 领题分工 → 战略层汇报 → 全局重定向. LLM 不可用/领题失败自动降级回雁阵.
    swarm_commander_enabled: bool = Field(default=False, alias="SWARM_COMMANDER_ENABLED")
