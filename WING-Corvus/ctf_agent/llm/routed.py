...
策略 (Sprint 33 三级路由: zen → go → 官方 flash):
...
5. (Sprint 19) 支持 model_tier="pro" 模式, 直接使用 pro 模型 (Sprint 33 起官方仅用 flash)
...
Sprint 19 新增:
- model_tier="pro" 跳过 flash, 使用 deepseek-v4-pro 求更高成功率 (Sprint 33: 改为官方 flash)
...
_GO_DEFAULT_TIMEOUT = 90.0    # Sprint 33: Opencode go 付费层, 与 zen 同级超时
...
class RoutedLLMClient:
    ...
    def _should_try_zen(self) -> bool:
        ...
        if self.settings.llm_provider == "go":
            return False
        # Sprint 33: DISABLE_ZEN=true 时跳过 zen 免费层, 直接路由至 go
    ...
    def _get_zen_client(self, timeout: float = 15.0) -> OpenAI | None:
        ...
        if self._zen_client is None:
            self._zen_client = OpenAI(
                ...
                # 走代理会绕境外 IP 导致请求慢/超时 (案例: #2664 每步 LLM 卡 90s+)
    ...
    def _get_go_client(self, timeout: float = 45.0) -> OpenAI | None:
        """获取 Opencode go client (Sprint 33: 付费 deepseek-v4-flash, 无峰谷)."""
        import httpx
    ...
    def _call_flash(self, payload: dict[str, Any], timeout: float | None) -> ChatResult:
        """flash 路由: zen → go → 官方 flash → pro (Sprint 33 + Sprint 32.8 动态降级)."""
        ...
        # Phase 2 (Sprint 33): Opencode go 付费层 (deepseek-v4-flash, 定价与官方一致且无峰谷).
        ...
        # Phase 3: 官方 flash fallback (如果启用) — Sprint 21: 失败重试 llm_max_retries 次,
        ...
        # Phase 4 (Sprint 32.8): pro 兜底 — fallback 也失败时降级到 pro, 而非直接抛异常.
        # Sprint 33: pro 模型改为官方 flash (官方仅使用 flash 模型).
