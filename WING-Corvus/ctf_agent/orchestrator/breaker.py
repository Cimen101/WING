...
class CircuitBreaker:
    ...
    def __init__(
        ...
        *,
        ...
        # Sprint 36 复盘: flash 模型极便宜, $1.5 成本熔断形同虚设
        # (threshold 单题烧 9.4M tokens 才 ~$1.15, 未触发熔断).
        # 增加 token 预算软/硬熔断: 超软限 → 注入收敛提示 (不终止);
        # 超硬限 → 终止 (防失控烧光 go 套餐 5 小时额度).
        max_tokens_soft: float = 2_500_000,
        max_tokens_hard: float = 4_000_000,
        ...
        self._hinted_tokens: bool = False  # Sprint 36: token 软预算提示已注入
    ...
    def check(self, step: ReActStep) -> BreakerAction:
        ...
        # 3.5. token 预算熔断 (Sprint 36: 软限注入收敛提示, 硬限终止)
        # flash 便宜使 $1.5 成本熔断失效 (9.4M tokens ≈ $1.15), 直接按 token 数熔断.
        if self.max_tokens_hard > 0 and self._accumulated_tokens > self.max_tokens_hard:
            return BreakerAction(
                action="terminate",
                reason=(
                    f"token 预算熔断：累计 {self._accumulated_tokens:,} tokens "
                    f"超过硬上限 {self.max_tokens_hard:,.0f}"
                ),
            )
        if (
            self.max_tokens_soft > 0
            and self._accumulated_tokens > self.max_tokens_soft
            and not self._hinted_tokens
        ):
            self._hinted_tokens = True
            return BreakerAction(
                action="inject_hint",
                message=(
                    f"⚠️ token 预算已消耗 {self._accumulated_tokens:,} tokens "
                    f"(软上限 {self.max_tokens_soft:,.0f}). 请立即收敛: "
                    f"停止低价值探索 (重复读源码/反复试错), 聚焦最有希望的 1-2 条路径, "
                    f"尽快产出可利用结果或 flag."
                ),
            )
