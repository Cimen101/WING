...
class Skill:
    ...
    confidence: str = ""  # high / medium / low, 空串时按 success_count 自动推导

    @property
    def effective_confidence(self) -> str:
        """confidence 实际值: 显式设置优先, 否则按 success_count 推导.

        - success_count >= 5 → high (多题验证)
        - success_count >= 1 → medium (至少成功过一次)
        - success_count == 0 → medium (手动策展的 skill, 质量可靠但未经自动验证)
        """
        if self.confidence:
