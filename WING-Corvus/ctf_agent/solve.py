...
输入 (task JSON, 见 readme):
    {
      "challenge_id": "challenge_001",   // 题目 ID
      "title": "题目名",
...
def _make_submission_handler() -> Any:
    """/30: 构造 submission_handler 回调 (从队列读取响应, 无 stdin 竞争).
...
def _build_engine(task: dict[str, Any], settings: Any):
    ...
    engine = ReActEngine(
        ...
        coordinator=_make_coordinator(
            ...
            # 战略层能力 — 总指挥协作 (commander_enabled 由
...
def _make_coordinator(
    ...
    Args:
        ...
        check_interval: 巡查间隔 (None=按风格节奏 保守10/中立8/激进5/创新8;
                        显式值全局覆盖, 范围钳制 5~10)
        bus/bus_challenge_id/commander_enabled: 总指挥协作 (战略层能力).
            默认关闭 → 纯雁阵行为不变.
    """
