...
class EcdsaNonceReuseTool(Tool):
    ...
    description = (
        ...
        "用法: ecdsa_nonce_reuse(curve='secp256k1', msg1='消息原文1', r1='...', s1='...', "
        "msg2='消息原文2', r2='...', s2='...', hash_algo='sha256', "
        "aes_nonce='hex', aes_ciphertext='hex', "
        "aes_key_mode='sha256')\n"
        "输入: r/s 为十进制字符串 (0x 前缀自动处理). z 是消息哈希的整数; "
        "提供 msg1/msg2 原始消息时自动按 hash_algo 计算 z, 无需手算.\n"
        "hash_algo: sha256 (默认) / sha1 / sha512 / raw (此时必须直接传 z1/z2 整数)\n"
        ...
        "容器/Kali 上 pycryptodome 已装, 无需 sagemath."
    )
    parameters = {
        "type": "object",
        "properties": {
            "curve": {"type": "string", "description": "曲线名 (secp256k1 / P-256)"},
            "z1": {"type": "string", "description": "(可选) 签名1 的消息哈希 (十进制或 0x); 提供了 msg1 时自动计算, 可不填"},
            "r1": {"type": "string", "description": "签名1 的 r (十进制或 0x)"},
            "s1": {"type": "string", "description": "签名1 的 s (十进制或 0x)"},
            "z2": {"type": "string", "description": "(可选) 签名2 的消息哈希 (十进制或 0x); 提供了 msg2 时自动计算, 可不填"},
            "r2": {"type": "string", "description": "签名2 的 r (十进制或 0x)"},
            "s2": {"type": "string", "description": "签名2 的 s (十进制或 0x)"},
            "hash_algo": {
                "type": "string",
                "description": "z 消息的 hash 算法: sha256 (默认) / sha1 / sha512 / raw (raw 时必填 z1/z2 整数)",
            },
            "msg1": {
                "type": "string",
                "description": "(可选) 签名1 的原始消息 (字符串), 提供后自动按 hash_algo 计算 z1",
            },
            "msg2": {
                "type": "string",
                "description": "(可选) 签名2 的原始消息 (字符串), 提供后自动按 hash_algo 计算 z2",
            },
            "aes_nonce": {
                "type": "string",
    ...
    def execute(
        ...
        **_: Any,
        ...
        try:
            ...
            # z1/z2: 提供了 msg 时按 hash_algo 计算; 否则直接使用传入的 z (必填其一)
            # WING-Goose (2026-08): 修复 z1/z2 必填导致 agent 只传 msg 时 TypeError
            if msg1 and hash_algo != "raw":
                z1_v = int.from_bytes(_hash_msg(msg1, hash_algo), "big")
            elif z1:
                z1_v = self._parse_int(z1)
            else:
                return ("ERROR: 缺少签名1 的消息: 请提供 msg1 (原始消息字符串) "
                        "或 z1 (消息哈希整数).")
            if msg2 and hash_algo != "raw":
                ...
                return ("ERROR: 缺少签名2 的消息: 请提供 msg2 (原始消息字符串) "
                        "或 z2 (消息哈希整数).")
