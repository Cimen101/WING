...
WAF bypasses: XML entity encoding (`&#x55;NION`), EXIF metadata injection (`exiftool -Comment="' UNION SELECT..."`), Shift-JIS `\u00a5`→`0x5c` backslash, QR code payload injection, double-keyword nesting (`selselectect`). See [database_query_patterns.md](database_query_patterns.md) for all techniques.

MySQL session variable dual-value injection: `@var:=` assigns return different values across sequential queries in one connection. PHP PCRE backtrack limit WAF bypass: 1M+ chars cause `preg_match()` to return `false`, passing `!false`. `information_schema.processlist` race condition leaks secrets from concurrent queries. See [database_query_patterns.md](database_query_patterns.md).

See [dynamic_module_processing.md](dynamic_module_processing.md) for PHP preg_replace /e RCE and Prolog injection. See [server-side-exec-2.md](server-side-exec-2.md) for SQLi via DNS records and SQLi keyword fragmentation.
...
**Windows 8.3 short filename bypass:** `FILEFO~1.EXT` short names bypass path filters that check the long filename. See [service_interaction_patterns.md](service_interaction_patterns.md#windows-83-short-filename-path-traversal-bypass-tokyo-westerns-2016).

**URL parse_url @ bypass:** `http://valid@attacker.com/` -- PHP `parse_url()` extracts `attacker.com` as host, bypassing domain checks. See [service_interaction_patterns.md](service_interaction_patterns.md#url-parseurl-symbol-bypass-ekoparty-ctf-2016).
- **SSRF double-@ parse discrepancy:** `http://x:x@127.0.0.1:80@allowed.host/path` — `parse_url()` sees `allowed.host`, curl connects to `127.0.0.1`. Distinct from single-@ bypass. See [service_interaction_patterns.md](service_interaction_patterns.md#ssrf-via-parseurlcurl-url-parsing-discrepancy-33c3-ctf-2016).
...
**Mako SSTI (Python):** `${__import__('os').popen('id').read()}` — no sandbox, plain Python inside `${}` or `<% %>`. **Twig SSTI (PHP):** `{{['id']|map('system')|join}}` — distinguish from Jinja2 via `{{7*'7'}}` (Twig repeats string, Jinja2 returns 49). See [core_input_processing.md](core_input_processing.md#mako-ssti) and [core_input_processing.md](core_input_processing.md#twig-ssti).

**Quote filter bypass:** Use `__dict__.update(key=value)` — keyword arguments need no quotes. See [core_input_processing.md](core_input_processing.md#ssti-quote-filter-bypass-via-dictupdate-apoorvctf-2026).

**ERB SSTI (Ruby/Sinatra):** `<%= Sequel::DATABASES.first[:table].all %>` bypasses ERBSandbox variable-name restrictions via the global `Sequel::DATABASES` array. See [core_input_processing.md](core_input_processing.md#erb-ssti-sequeldatabases-bypass-bearcatctf-2026).

## Python str.format() Attribute Traversal (PlaidCTF 2017)

Python `str.format()` allows dot-notation attribute traversal (`{0.attr.subattr}`) and bracket indexing (`{0[key]}`). When user input reaches `.format(obj)`, leak arbitrary attributes without a template engine. Distinct from SSTI. See [core_input_processing.md](core_input_processing.md#python-strformat-attribute-traversal-plaidctf-2017).

**Thymeleaf SpEL SSTI (Java/Spring):** `${T(org.springframework.util.FileCopyUtils).copyToByteArray(new java.io.File("/flag.txt"))}` reads files via Spring utility classes when standard I/O is WAF-blocked. Works in distroless containers (no shell). See [dynamic_module_processing.md](dynamic_module_processing.md#thymeleaf-spel-ssti-spring-filecopyutils-waf-bypass-apoorvctf-2026).
...
**Host header SSRF:** Server builds internal request URL from `Host` header (e.g., `http.Get("http://" + request.Host + "/validate")`). Set Host to attacker domain → validation request goes to attacker server. See [core_input_processing.md](core_input_processing.md#host-header-ssrf-mireactf).

**ElasticSearch Groovy RCE via SSRF:** SSRF to internal ES on port 9200 enables RCE through `script_fields` Groovy scripting (pre-5.0). See [service_interaction_patterns.md](service_interaction_patterns.md#elasticsearch-groovy-scriptfields-rce-via-ssrf-volgactf-2017).
...
**Git CLI newline injection:** `%0a` in URL path breaks out of backtick/system() shell calls that only filter `;|&<>`. See [core_input_processing.md](core_input_processing.md#git-cli-newline-injection-via-url-path-bsidessf-2026).
...
**XXE in DOCX uploads:** DOCX is ZIP+XML; inject XXE in `[Content_Types].xml` inside the archive. See [core_input_processing.md](core_input_processing.md#xxe-via-docxoffice-xml-upload-school-ctf-2016).
...
See [core_input_processing.md](core_input_processing.md#php-type-juggling) for comparison table and exploit payloads.
...
See [core_input_processing.md](core_input_processing.md#php-file-inclusion-phpfilter) for filter chains and RCE techniques.
...
- **LaTeX restricted write18 bypass:** When `write18` is restricted, `mpost -ini "-tex=mock_bash -c (cmd)" file.mp` uses mpost's whitelisted status to execute arbitrary commands. `${IFS}` replaces spaces. See [service_interaction_patterns.md](service_interaction_patterns.md#latex-rce-via-mpost-restricted-write18-bypass-33c3-ctf-2016).

**PHP backtick eval (character limit):** `` echo`cat *`; `` -- PHP backticks = `shell_exec()`, fits RCE in as few as 8 chars. Use `` `$_GET[0]`; `` to move payload to URL parameter. See [dynamic_module_processing.md](dynamic_module_processing.md#php-backtick-eval-under-character-limit-easyctf-2017).
**PHP assert() injection:** `assert("strpos('$input', '..') === false")` — inject `') || system('cmd');//` for RCE (PHP < 7.2). See [dynamic_module_processing.md](dynamic_module_processing.md#php-assert-string-evaluation-injection-csaw-ctf-2016).
...
**Ruby ObjectSpace scanning:** `ObjectSpace.each_object(String)` dumps all in-memory strings including flag. See [dynamic_module_processing.md](dynamic_module_processing.md#ruby-objectspace-memory-scanning-for-flag-extraction-tokyo-westerns-2016).

See [dynamic_module_processing.md](dynamic_module_processing.md) for full payloads and bypass techniques.

## Java Deserialization

Serialized Java objects (`rO0AB` / `aced0005`) + ysoserial gadget chains → RCE via `ObjectInputStream.readObject()`. Try `CommonsCollections1-7`, `URLDNS` for blind detection. See [object_mapper_logic.md](object_mapper_logic.md#java-deserialization-ysoserial).

## Python Pickle Deserialization

`pickle.loads()` calls `__mock_reduce__()` → `(mock_os_system, ('cmd',))` instant RCE. Also via `yaml.load()`, `torch.load()`, `joblib.load()`. See [object_mapper_logic.md](object_mapper_logic.md#python-pickle-deserialization).

## Race Conditions (Time-of-Check to Time-of-Use)

Concurrent requests bypass check-then-act patterns (balance, coupons, registration). Send 50 simultaneous requests — all see pre-modification state. See [object_mapper_logic.md](object_mapper_logic.md#race-conditions-time-of-check-to-time-of-use).
...
See [dynamic_module_processing.md](dynamic_module_processing.md) and [server-side-exec-2.md](server-side-exec-2.md) for detailed steps.
...
SSRF to unauthenticated Docker daemon on port 2375. Use `/archive` for file extraction, `/exec` + `/exec/{id}/start` for command execution. Chain through internal POST relay when SSRF is GET-only. See [service_interaction_patterns.md](service_interaction_patterns.md#ssrf-to-docker-api-rce-chain-h7ctf-2025).

## Castor XML Deserialization via xsi:type (Atlas HTB)

Castor XML `Unmarshaller` without mapping file trusts `xsi:type` attributes for arbitrary Java class instantiation. Chain through JNDI (Java Naming and Directory Interface) / RMI (Remote Method Invocation) via ysoserial `CommonsBeanutils1` for RCE. Requires Java 11 (not 17+). Check `pom.xml` for `castor-xml`. See [service_interaction_patterns.md](service_interaction_patterns.md#castor-xml-deserialization-via-xsitype-polymorphism-atlas-htb).

## Apache ErrorDocument Expression File Read (Zero HTB)

`.htaccess` with `ErrorDocument 404 "%{file:/etc/passwd}"` reads files at Apache level, bypassing `php_admin_flag engine off`. Requires `AllowOverride FileInfo`. Upload via SFTP, trigger with 404 request. See [service_interaction_patterns.md](service_interaction_patterns.md#apache-errordocument-expression-file-read-zero-htb).
...
Strip pickle STOP opcode (`\x2e`) from first payload, concatenate second — both `__mock_reduce__` calls execute in single `pickle.loads()`. Chain `os.dup2()` for socket output. See [object_mapper_logic.md](object_mapper_logic.md#pickle-chaining-via-stop-opcode-stripping-volgactf-2013).

## XPath Blind Injection (BaltCTF 2013)

`substring(normalize-space(../../../node()),1,1)='a'` — boolean-based blind extraction from XML data stores via response length oracle. See [dynamic_module_processing.md](dynamic_module_processing.md#xpath-blind-injection-baltctf-2013).

## SQLite File Path Traversal to Bypass String Equality (Codegate 2013)

Input `/../gamesim_GM` fails `== "GM"` string check but filesystem normalizes `/var/game_db/gamesim_/../gamesim_GM.db` to the blocked path. See [service_interaction_patterns.md](service_interaction_patterns.md#sqlite-file-path-traversal-to-bypass-string-equality-codegate-2013).

## PHP Serialization Length Manipulation via Filter Word Expansion (0CTF 2016)

Post-serialization string filter replaces "where" (5 chars) with "hacker" (6 chars). Repeat "where" N times so expansion overflows by exactly enough bytes to inject a serialized field (`";}s:5:"photo";s:10:"config.php";}`). See [object_mapper_logic.md](object_mapper_logic.md#php-serialization-length-manipulation-via-filter-word-expansion-0ctf-2016).
...
Server builds XML from headers without escaping. Inject `</ip><admin>true</admin><ip>` via X-Forwarded-For; first-tag-wins XML parsing. See [core_input_processing.md](core_input_processing.md#xml-injection-via-x-forwarded-for-header-pwn2win-2016).
