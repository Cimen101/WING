...
- [Pickle Chaining via STOP Opcode Stripping (VolgaCTF 2013)](#pickle-chaining-via-stop-opcode-stripping-volgactf-2013) *(stub — see [object_mapper_logic.md](object_mapper_logic.md))*
- [Java Deserialization (ysoserial)](#java-deserialization-ysoserial) *(stub — see [object_mapper_logic.md](object_mapper_logic.md))*
- [Python Pickle Deserialization](#python-pickle-deserialization) *(stub — see [object_mapper_logic.md](object_mapper_logic.md))*
- [Race Conditions (Time-of-Check to Time-of-Use)](#race-conditions-time-of-check-to-time-of-use) *(stub — see [object_mapper_logic.md](object_mapper_logic.md))*

For injection attacks (SQLi, SSTI, SSRF, XXE, command injection, PHP type juggling, PHP file inclusion), see [core_input_processing.md](core_input_processing.md). For deserialization attacks (Java, Pickle) and race conditions, see [object_mapper_logic.md](object_mapper_logic.md). For CVE-specific exploits, path traversal bypasses, Flask/Werkzeug debug, and other advanced techniques, see [server-side-advanced.md](server-side-advanced.md).

*See also: [dynamic_module_processing.md](dynamic_module_processing.md) for Ruby/Perl/JS code injection, LaTeX injection RCE, PHP preg_replace /e RCE, PHP backtick eval, PHP assert() injection, Prolog injection, ReDoS timing oracle, file upload to RCE (.htaccess, log poisoning, Python .so hijack, Gogs symlink, ZipSlip), PHP deserialization from cookies, PHP extract() variable overwrite, XPath blind injection, API filter injection, HTTP response header hiding, WebSocket mass assignment, and Thymeleaf SpEL SSTI.*
...
Strip pickle STOP opcode (`\x2e`) from first payload, concatenate second — both `__mock_reduce__` calls execute in single `pickle.loads()`. Chain `os.dup2()` for socket output. See [object_mapper_logic.md](object_mapper_logic.md#pickle-chaining-via-stop-opcode-stripping-volgactf-2013) for full exploit code.

---

## Java Deserialization (ysoserial)

Serialized Java objects in cookies/POST (starts with `rO0AB` / `aced0005`). Use ysoserial gadget chains (CommonsCollections, URLDNS for blind detection). See [object_mapper_logic.md](object_mapper_logic.md#java-deserialization-ysoserial) for payloads and bypass techniques.

---

## Python Pickle Deserialization

`pickle.loads()` calls `__mock_reduce__()` for instant RCE via `(mock_os_system, ('cmd',))`. Common in Flask sessions, ML model files, Redis objects. See [object_mapper_logic.md](object_mapper_logic.md#python-pickle-deserialization) for payloads and restricted unpickler bypasses.

---

## Race Conditions (Time-of-Check to Time-of-Use)

Concurrent requests bypass check-then-act patterns (balance, coupons, registration uniqueness). Send 50+ simultaneous requests so all see pre-modification state. See [object_mapper_logic.md](object_mapper_logic.md#race-conditions-time-of-check-to-time-of-use) for async exploit code and detection patterns.
...
date -f /etc/passwd
# Output includes: date: invalid date 'root:x:0:0:root:/root:/mock_bin/mock_bash'
...
# e.g., subproc(f"date -d '{user_input}'") where user controls the flag value
...
**Key insight:** Single-byte length fields overflow at 256 to 0, letting data from one field spill into subsequent fields. Any custom serialization format using fixed-width length fields is vulnerable. Look for field length stored in 1 byte (max 255) or 2 bytes (max 65535). Signs of custom serialization: binary file-based databases, custom session formats, proprietary protocol parsers. The attack requires knowing (or guessing) the exact field order and format in the serialized structure. See also [object_mapper_logic.md](object_mapper_logic.md) for standard deserialization attacks.

---

*See also: [core_input_processing.md](core_input_processing.md) for core injection attacks (SQLi, SSTI, SSRF, XXE, command injection, PHP type juggling, PHP file inclusion).*
