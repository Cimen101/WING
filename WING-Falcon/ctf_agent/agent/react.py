...
def parse_llm_output(text: str) -> ParsedAction:
    ...
    if action_match and not input_match:
        json_match = _RE_JSON_OBJECT.search(text)
        if json_match:
            class _Stub:
                pass
            stub = _Stub()
            stub.group = lambda _i=1: json_match.group(0)
            input_match = stub
    if action_match and input_match:
        ...
        action_input = _strip_code_fence(input_match.group(1))
        return ParsedAction(
            thought=thought,
            action=action,
...
class ReActEngine:
    ...
    def _run_inner(self, task: str) -> ReActResult:
        ...
        if self._mid_term is not None:
            ...
            self.tools = {t.name: t for t in extended}
        ...
        # RAG 检索：任务开始时一次性检索相似历史方案（不每轮刷新，节省 LLM 调用）
        rag_context = ""
        if self._long_term is not None:
            ...
            rag_context = retriever.retrieve(task)
        ...
        while True:
            ...
            # 巡查指导器 — 每 10/15 步巡查一次 (LLM 驱动的智能旁观者)
            if self._coordinator is not None and self._coordinator.should_check(step_no, self.max_steps):
                try:
                    # 构造轨迹数据给 coordinator 分析 (含 observation, 供 LLM 深度分析)
                    traj = [
                        {
                    ...
                    guidance = self._coordinator.analyze(
                        traj,
                        challenge_type=self._challenge_type or "",
                    ...
                    if self._on_coordinator is not None:
                        try:
                            self._on_coordinator(guidance, step_no)
                        except Exception:
                            pass  # 日志异常不影响主流程
                except Exception:
                    pass  # 巡查异常不影响主流程
            ...
            if self._coordinator_guidance:
                if self._must_repeat_left > 0:
                    ...
                    memory.add_user_message(self._coordinator_guidance + persist_hint)
                    self._must_repeat_left -= 1
                else:
                    memory.add_user_message(self._coordinator_guidance)
