...
class ReActEngine:
    ...
    def __init__(
        ...
        *,
        ...
        challenge_type: str | None = None, # cross-challenge 知识共享
    ...
    def _run_inner(self, task: str) -> ReActResult:
        ...
        # 仅当外部 system prompt 已传入时才追踪 (build_system_prompt 内部调 _try_inject_skills)
        # sys_prompt=None 时无静态注入, _injected_exp_ids 保持空集, 所有经验可 mid-solve 注入
        ...
        while True:
            ...
            if parsed.is_final:
                ...
                self.status.mark_done(parsed.final_answer)
                # 成功后清理失败历史(避免成功解出后再次跑时还注入旧的失败提示)
                self._clear_failed_if_solved()
                return ReActResult(
                    success=True,
                    final_answer=parsed.final_answer,
