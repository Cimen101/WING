...
策略 (三级路由: zen → go → 官方 flash):
1. 优先使用 zen endpoint (opencode.ai, 免费 deepseek-v4-flash-free), 可通过 DISABLE_ZEN 关闭
2. 次选 Opencode go (付费 deepseek-v4-flash, 定价与官方一致, 无峰谷收费倍率)
3. go 失败时重试 N 次 (默认 2)
4. 仍失败则回退到官方 deepseek-v4-flash
5. 支持 model_tier="pro" 模式, 直接使用 pro 模型 (起官方仅用 flash)
...
新增:
- model_tier="pro" 跳过 flash, 使用 deepseek-v4-pro 求更高成功率 (改为官方 flash)
...
_ZEN_DEFAULT_TIMEOUT = 45.0 # v5: httpx.Timeout 客户端级防卡死, 45s 避免误杀正常请求
_GO_DEFAULT_TIMEOUT = 90.0 # Opencode go 付费层, 与 zen 同级超时
                              # WING-Goose (2026-08): 45→90 — thinking 长推理常超 45s,
                              # 45s 硬超时会触发 3 轮重试链 (45*3+2*2=139s 白扔), 放宽后长思考一次成功
...
_GO_WALLCLOCK = 90.0    # WING-Goose (2026-08): go 分支专用硬超时 — thinking 长推理生成 45s+
                        # 很常见, 45s 硬超时 → 3 轮重试链白扔 ~139s (见 历史复盘);
...
class RoutedLLMClient:
    ...
    支持 model_tier:
        "flash" (默认): zen → go → 官方 flash fallback
        "pro":   zen → go → 官方 flash → pro fallback
    ...
    def __init__(self, settings: Settings | None = None) -> None:
        ...
        self._go_client: OpenAI | None = None # Opencode go 付费层
    ...
    def _should_try_zen(self) -> bool:
        # WING-Goose (2026-08): LLM_PROVIDER=go → 只走 go 套餐, 永远不试 zen
        if self.settings.llm_provider == "go":
            return False
        # DISABLE_ZEN=true 时跳过 zen 免费层, 直接路由至 go
        if self.settings.disable_zen:
            return False
    ...
    def apply_smoke_from_file(self, path: str) -> None:
        ...
        try:
            ...
            ok: dict[str, bool] = {}
            for key in ("zen", "go", "flash_fallback", "pro"):
                v = data.get(key)
    ...
    def smoke_test(self, timeout: float = 10.0) -> dict[str, bool]:
        ...
        # WING-Goose (2026-08): LLM_PROVIDER=go → 只探测 go (国内部署直连, 单次 10s 超时),
        # 不浪费 10s×N 探测其他 provider. 领题前 0.6 检查据此决定是否报错退出.
        if self.settings.llm_provider == "go":
            results["zen"] = False
            results["flash_fallback"] = False
            results["pro"] = False
            try:
                results["go"] = _probe(self._get_go_client(timeout=timeout), self.settings.go_model, timeout)
            except Exception:
                results["go"] = False
    ...
    def _get_zen_client(self, timeout: float = 15.0) -> OpenAI | None:
        ...
        if self._zen_client is None:
            self._zen_client = OpenAI(
                ...
                # WING-Goose (2026-08): 国内部署模型必须直连 (禁系统代理),
                # 走代理会绕境外 IP 导致请求慢/超时 (案例: 每步 LLM 卡 90s+)
                http_client=httpx.Client(proxy=None, trust_env=False,
                                         timeout=httpx.Timeout(timeout, connect=5, read=timeout)),
            )
        return self._zen_client

    def _get_go_client(self, timeout: float = 45.0) -> OpenAI | None:
        """获取 Opencode go client (付费 deepseek-v4-flash, 无峰谷)."""
        import httpx
        key = self.settings.go_api_key.get_secret_value()
        if not key:
            return None
        if self._go_client is None:
            self._go_client = OpenAI(
                api_key=key,
                base_url=self.settings.go_base_url,
                timeout=httpx.Timeout(timeout, connect=5, read=timeout),
                # WING-Goose (2026-08): go 套餐已启用国内部署 → 必须直连国内 IP
                # (禁系统代理), 响应快且稳定. 历史复盘: 走系统代理
    ...
    def _get_fallback_client(self, timeout: float = 30.0) -> OpenAI | None:
        ...
        if self._fallback_client is None:
            ...
            self._fallback_client = OpenAI(
                ...
                # WING-Goose (2026-08): 官方 deepseek 国内可达, 直连禁代理
                http_client=httpx.Client(proxy=None, trust_env=False,
                                         timeout=httpx.Timeout(timeout, connect=10, read=timeout)),
            )
    ...
    def _call_flash(self, payload: dict[str, Any], timeout: float | None) -> ChatResult:
        """flash 路由: zen → go → 官方 flash → pro （ 动态降级)."""
        # WING-Goose (2026-08): LLM_PROVIDER=go → 只走 go 套餐, go 失败直接抛
        # (禁用官方/pro 降级链; go 国内部署直连, 领题前冒烟已保证 ≤10s 响应)
        go_only = self.settings.llm_provider == "go"
        used_model_zen = self.settings.zen_model
        used_model_go = self.settings.go_model
        used_model_fallback = self.settings.fallback_model
        zen_timeout = timeout if timeout is not None else _ZEN_DEFAULT_TIMEOUT
        go_timeout = timeout if timeout is not None else _GO_DEFAULT_TIMEOUT
        ...
        # Phase 2 : Opencode go 付费层 (deepseek-v4-flash, 定价与官方一致且无峰谷).
        # 余额充足 (60USD) → 调试阶段主用通道. 与 zen 同策略: 动态健康 + wall-clock 超时.
        if self._provider_healthy("go"):
            go_client = self._get_go_client(timeout=go_timeout)
            if go_client is not None:
                go_payload = {**payload, "model": used_model_go, "timeout": go_timeout}
                for attempt in range(self.settings.llm_max_retries + 1):
                    try:
                        resp = _call_with_wallclock(
                        ...
                        return _parse_response(resp, used_model_go)
                    except Exception as e:
                        status_code = getattr(e, "status_code", None)
                        if status_code is not None and 500 <= status_code < 600:
                            break  # 5xx 不重试, 直接降级官方
                        if attempt == self.settings.llm_max_retries:
                            break
                        time.sleep(2)
                # go 尝试完毕且失败 → 记录动态故障, 进入跳过期
                self._record_provider_fail("go")

        # WING-Goose (2026-08): go-only 模式下 go 失败直接抛错, 不降级官方/pro
        if go_only:
            raise RuntimeError("go 套餐调用失败 (LLM_PROVIDER=go 禁用降级), 请检查网络/API 状态")

        # Phase 3: 官方 flash fallback (如果启用) — 失败重试 llm_max_retries 次,
        # 避免单次网络抖动直接抛异常导致整题 0 步失败 (NSS 复盘)
        ...
        # Phase 4 : pro 兜底 — fallback 也失败时降级到 pro, 而非直接抛异常.
        # pro 模型改为官方 flash (官方仅使用 flash 模型).
    ...
    def chat(
        ...
        Args:
            model_tier: "flash" (default) | "pro" (skip flash) | "pro_only"
                ⚠️ pro/pro_only 已 deprecated, 默认不推荐使用
        ...
        if self.settings.enable_thinking_mode:
            ...
            # 2026-08-03 根因修复 (go/opencode 端点):
            #   deepseek-v4-flash 2026-07-31 升级为 reasoning 模型, go 端点为 Agent 类请求
            #   思考链强制无界生成, 非 none 的 effort 一律撞满 max_tokens (74-86s/次, content=0),
            #   提示词压缩 / thinking disabled / effort 分级全部无效 (9 组受控实验定论).
            #   唯一可控档 = "none": 思考归零, content 正常, 每步 10-20s (推理由 ReAct Thought 承担).
            if self.settings.llm_provider == "go":
                payload["reasoning_effort"] = "none"
                # thinking 扩展参数在 go 端点无效 (enabled/disabled 均不改变行为), 不再注入
                existing_extra_body = extra.pop("extra_body", None) or {}
                if existing_extra_body:
                    payload["extra_body"] = existing_extra_body
            else:
