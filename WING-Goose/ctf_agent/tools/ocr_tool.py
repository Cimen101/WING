...
class OcrTool(Tool):
    ...
    parameters = {
        ...
        "properties": {
            "file_path": {
                "type": "string",
                "description": "Kali 上图片/文件的绝对路径 (如 /tmp/ctf_workspace/image.jpeg)",
            },
            "lang": {
                "type": "string",
                "description": (
    ...
    def execute(self, file_path: str, lang: str = "eng", psm: int = 3, **_: Any) -> str:
        ...
        if not actual:
            return (
                ...
                f"[NO_TEXT_DETECTED] 图片无文字, 不要拿 OCR 输出做 web_search query.\n"
        ...
        if _is_likely_no_text(actual):
            return (
                f"[NO_TEXT_DETECTED] OCR 未提取出真实文字 (只输出 {len(actual)} 字符 tesseract 内部日志).\n"
                f"原始输出: {actual[:200]}\n"
